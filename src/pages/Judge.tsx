...
const Judge = () => {
  ...
  const [totalApplications, setTotalApplications] = useState(3);
  const [pendingReviews, setPendingReviews] = useState(2);
  const [reviewProgress, setReviewProgress] = useState(33); // percentage
  ...
  const [applications, setApplications] = useState([
    {
      ...
      founder: 'Emily Chen',
      description: 'A sustainable technology platform for reducing carbon emissions in industrial processes through AI optimization.',
      event: 'TechStars Startup Weekend 2023',
      submittedDate: '2023-10-05',
      status: 'pending',
      score: null,
      averageScore: 60.0,
      attachments: ['Pitch Deck.pdf', 'Financial Projections.pdf', 'Team Overview.pdf'],
      evaluation: {
    ...
    },
    {
      ...
      description: 'Telemedicine platform connecting patients with specialists worldwide for remote consultations and follow-ups.',
      event: 'TechStars Startup Weekend 2023',
      submittedDate: '2023-10-06',
      status: 'pending',
      score: null,
      averageScore: 60.0,
      attachments: ['MediConnect Deck.pdf', 'Market Analysis.pdf', 'Product Demo.mp4'],
      evaluation: {
        problemSolution: { score: 3, weight: 25 },
        marketPotential: { score: 3, weight: 20 },
        executionStrategy: { score: 3, weight: 20 },
    ...
    },
    {
      ...
      description: 'AI-powered personalized learning platform for K-12 students, adapting to individual learning styles and pace.',
      event: 'Google for Startups Accelerator',
      submittedDate: '2023-09-28',
      status: 'reviewed',
      score: 87,
      feedback: 'Strong product with clear market fit. Team has relevant experience. Would benefit from more traction metrics.',
      reviewedDate: '2023-12-05'
    }
  ]);
  
  // Function to update evaluation scores
  const updateEvaluation = (applicationId: number, criteriaKey: keyof Evaluation, newScore: number): void => {
      ...
      prevApplications.map(app => {
        if (app.id === applicationId && app.evaluation) {
          ...
          Object.values(updatedEvaluation).forEach(criteria => {
            if (criteria && typeof criteria.score === 'number' && typeof criteria.weight === 'number') {
              totalWeightedScore += criteria.score * criteria.weight;
              totalWeight += criteria.weight;
            }
          });
          
          const newAverageScore = totalWeight > 0 ? totalWeightedScore / totalWeight : 0;
  ...
  const renderApplicationCards = () => {
                  ...
                  {filteredApplications.map((app) => (
                    <tr key={app.id} className="border-b hover:bg-gray-50">
                      <td className="px-4 py-3">
                        ...
                        <div className="text-gray-500 text-xs">{app.founder}</div>
                      </td>
    ...
    return (
      <div className="mt-4 space-y-6">
        {filteredApplications.map((app) => (
          <div key={app.id} className="bg-white rounded-lg border shadow-sm overflow-hidden">
            <div className="p-6">
              <div className="flex flex-col lg:flex-row justify-between">
                ...
                {app.status === 'reviewed' ? (
                  ...
                  <div className="mt-2 lg:mt-0 text-right">
                    <div className="text-3xl font-bold">{app?.averageScore?.toFixed(1)}</div>
              ...
              {app.status === 'reviewed' && app.feedback && (
                <div className="mt-4">
                  ...
                  <p className="text-gray-500 text-sm mt-2">Reviewed: {app.reviewedDate}</p>
                </div>
              )}
                ...
                <>
                  <div className="mt-4">
                    ...
                    <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                      {app?.attachments?.map((attachment, index) => (
                        <div key={index} className="flex items-center p-2 rounded border bg-gray-50">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="space-y-4">
                      {app.evaluation && Object.entries(app.evaluation).map(([key, value]) => {
                        ...
                        return (
                          <div key={key}>
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-gray-500">Weight: {value.weight}%</span>
                            </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8 mt-16">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Review and evaluate startup applications.</p>
        </div>
        ...
        {/* Tabs */}
        <div className="mb-4">
          <div className="inline-flex rounded-md shadow-sm bg-gray-100 p-1" role="tablist">
            <button
